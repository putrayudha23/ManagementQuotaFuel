...
def encrypt_script_endpoint(request):
    if request.method == 'POST':
        ...
        try:
            ...
            ccrypt_command = ['ccrypt', '-e', '-K', passphrase, gzipped_path]
            subprocess.run(ccrypt_command, check=True)

            ccrypt_command_adhoc = ['ccrypt', '-e', '-K', passphrase, adhoc_path]
            subprocess.run(ccrypt_command_adhoc, check=True)
